...
from torch_geometric.nn import SGConv,SAGEConv,GCNConv,GATConv,GraphSAGE,GraphConv
from .model import (
    MVCDecoder,
...
class TransformerGenerator(nn.Module):
    def __init__(
        ...
        self.pert_Embedding = nn.Embedding(genes+2, d_model, padding_idx=pert_pad_id)
        self.num_perts = num_perts
        self.G_sim = G_sim
        self.G_sim_weight = G_sim_weight
        self.n_GNN =n_GNN
        self.pert_encoder = GO_Graph_Encoder(self.num_perts, d_model, self.n_GNN,self.G_sim, self.G_sim_weight)
        self.pert_fuse = MLP([d_model, d_model, d_model], last_layer_act='ReLU')
    ...
    def _encode(
        ...
        perts = self.pert_Embedding(input_pert_flags)
        total_embs = src + values +perts
        #total_embs = total_embs.reshape(values.shape[0],values.shape[1], -1)

        # add GNN
        pert_global_emb = self.pert_encoder(values.device)  # (batch, seq_len, embsize)
        # for i in range(len(pert_idx)):
        #     for pert in pert_idx[i]:
        #         if pert != -1:
        #             total_embs[i]=total_embs[i]+pert_global_emb[pert]
        pert_index = []
        ...
        if pert_index.shape[0] != 0:
            ...
            if len(list(pert_track.values())) > 0:
                ...
                for idx, j in enumerate(pert_track.keys()):
                    total_embs[j] = total_embs[j] + emb_total[idx] 
    ...
    def forward(
        ...
        transformer_output = self._encode(
            src, values, input_pert_flags,pert_idx,src_key_padding_mask
        )
        ...
        #差值
        #mlm_output["pred"] = mlm_output["pred"] + values
    ...
    def pred_perturb(
        ...
        #print(x.shape)
        length =len(x) # Length of the one-dimensional tensor
        #pert_flags = torch.full((length,), 2)
        pert_flags = torch.zeros(len(x))
        ori_gene_values = x.view(batch_size, -1)
        n_genes=len(ori_gene_values[0])
        for i in range(0,len(batch_data.pert_idx)):
            for j in range(0,len(gene_pert_idx)):
                if len(batch_data.pert_idx[i]) == 1:
                    if gene_pert_idx[j][1] == batch_data.pert_idx[i][0]:
                        pert_flags[i*n_genes+gene_pert_idx[j][0]]=1
                    if batch_data.pert_idx[i][0]==-1:
                        pert_flags[i*n_genes:(i+1)*n_genes]=2
                        break
                
                if len(batch_data.pert_idx[i]) == 2:
                    
                    if gene_pert_idx[j][1] == batch_data.pert_idx[i][0]:
                        pert_flags[i*n_genes+gene_pert_idx[j][0]]=1
                    if gene_pert_idx[j][1] == batch_data.pert_idx[i][1]:
                        pert_flags[i*n_genes+gene_pert_idx[j][0]]=1
...
class GO_Graph_Encoder(nn.Module):
    def __init__(
        self,
        num_perts: int,
        embedding_dim: int,
        ...
        super().__init__()
        self.pert_emb = nn.Embedding(num_perts, embedding_dim,max_norm=True)
        self.sim_layers = torch.nn.ModuleList()
        for i in range(1, num_layers + 1):
          
            self.sim_layers.append(SGConv(embedding_dim, embedding_dim))
        self.G_sim = G_sim
        self.G_sim_weight = G_sim_weight
    ...
    def forward(self,device) -> Tensor:
        pert_global_emb = self.pert_emb(torch.LongTensor(list(range(self.num_perts))).to(device))        

            ## augment global perturbation embedding with GNN
        for idx, layer in enumerate(self.sim_layers):
            pert_global_emb = layer(pert_global_emb, self.G_sim.to(device), self.G_sim_weight.to(device))
            if idx < self.num_layers - 1:
                pert_global_emb = pert_global_emb.relu()
        
        return pert_global_emb
