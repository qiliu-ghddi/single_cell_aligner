...
from ..compound_processor import atom_features, get_atom_feature_dims, canonicalize_molecule, gpickle
...
class TransformerModel(nn.Module):
    def __init__(
        ...
        self.encoder = GeneEncoder(ntoken, d_model, padding_idx=vocab[pad_token]) ####Gene encoder

        self.atom_feature_dims = get_atom_feature_dims()
        self.comp_encoder = CompoundEncoder(self.atom_feature_dims, d_model)
        ...
        if use_fast_transformer:
            if fast_transformer_backend == "linear":
            ...
            elif fast_transformer_backend == "nonzero":  
                print("Use NonZeroTransformerEncoder")
                encoder_layers = NonZeroTransformerEncoderLayer(
                    d_model,
                    nhead,
                    d_hid,
                    dropout,
                ...
                self.transformer_encoder = NonZeroTransformerEncoder(encoder_layers, nlayers)
        else:
    ...
    def _encode(
        ...
        values_transformer = values.clone()
        values = self.value_encoder(values)  # (batch, seq_len, embsize)
        smiles_src = self.comp_encoder(smiles_src,seq_len=values.shape[1],device=next(self.parameters()).device)
        ...
        #     total_embs = src + values
            ...
            total_embs = src + values +  smiles_src
            # total_embs = src +  smiles_src
            # total_embs = src * smiles_src
        ...
        try:
            output = self.transformer_encoder(
                src=total_embs, values=values_transformer, src_key_padding_mask=src_key_padding_mask
            )
        except:
            output = self.transformer_encoder(
    ...
    def forward(
        ...
        output = {}
        first_mlp_features, second_mlp_features, third_mlp_features, mlm_output = self.decoder(
            transformer_output
            if not self.use_batch_labels
            else torch.cat(
                [
        ...
        if self.explicit_zero_prob and do_sample:
            ...
            output["mlm_output"] = mlm_output["pred"].float()  # (batch, seq_len)
        ...
        return smiles_embs, transformer_output, first_mlp_features, second_mlp_features, third_mlp_features, output
...
class NonZeroTransformerEncoderLayer(nn.Module):
    __constants__ = ["batch_first"]
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Create mask for values equal to zero
        zero_mask = (values.abs() < 1e-2)
        if src_key_padding_mask is not None:
            combined_mask = src_key_padding_mask | zero_mask
        else:
            combined_mask = zero_mask

        src_key_padding_mask_ = combined_mask
        # ###----for Debug ----###
        # # 打印 mask zero 中 False 的个数
        # num_false_in_mask = torch.sum(~zero_mask).item()
        # print("Number of False in mask zero:", num_false_in_mask)

        # # 打印 mask zero 中 True 的个数
        # num_true_in_mask = torch.sum(zero_mask).item()
        # print("Number of True in mask zero:", num_true_in_mask)
        # ###----for Debug ----###

        if self.norm_scheme == "pre":
            src = self.norm1(src)
            src2 = self.self_attn(src, key_padding_mask=src_key_padding_mask_)[0]
            expanded_zero_mask = zero_mask.unsqueeze(-1).expand_as(src)
            selected_elements = src.clone().masked_select(expanded_zero_mask).to(dtype=src2.dtype)
            src2.masked_scatter_(expanded_zero_mask, selected_elements)
            src = src + self.dropout1(src2)
            src = self.norm2(src)
            src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
            src = src + self.dropout2(src2)
            ...
            src2.masked_scatter_(expanded_zero_mask, selected_elements)
            # ###----for Debug ----###
            # # 对每个 batch 处理以打印对应的 src 和 src2 值
            # for b in range(1):
            #     # 对于 zero_mask 为 False 的位置
            #     false_positions = ~zero_mask[b]
            #     corresponding_src_values_false = src[b, false_positions, :3].tolist()
            #     corresponding_src2_values_false = src2[b, false_positions, :3].tolist()
            #     print(f"Batch {b}, Corresponding src values for False in mask zero (first 3 dimensions):", corresponding_src_values_false[:5])
            #     print(f"Batch {b}, Corresponding src2 values for False in mask zero (first 3 dimensions):", corresponding_src2_values_false[:5])

            #     # 对于 zero_mask 为 True 的位置
            #     true_positions = zero_mask[b]
            #     corresponding_src_values_true = src[b, true_positions, :3].tolist()
            #     corresponding_src2_values_true = src2[b, true_positions, :3].tolist()
            #     print(f"Batch {b}, Corresponding src values for True in mask zero (first 3 dimensions):", corresponding_src_values_true[:5])
            #     print(f"Batch {b}, Corresponding src2 values for True in mask zero (first 3 dimensions):", corresponding_src2_values_true[:5])
            # ###----for Debug ----###
...
class NonZeroTransformerEncoder(nn.Module):
    def __init__(self, encoder_layer, num_layers, **kwargs):
        super(NonZeroTransformerEncoder, self).__init__()
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers, **kwargs)

    def forward(self, src, values, src_key_padding_mask=None):
        # loop through the number of layers in encoder and apply the custom encoder layer
        for layer in self.encoder.layers:
            src = layer(src, values, src_key_padding_mask=src_key_padding_mask)
        return src
...
class CompoundEncoder(nn.Module):
    def __init__(
        ...
        atom_feature_dims,
        embedding_dim: int,
        atom_dict_file = 'data/atom_dict.gpkl',
        ...
        self.embeds = []
        for i, dim in enumerate(atom_feature_dims):
            emb = torch.nn.Embedding(dim, embedding_dim)
            torch.nn.init.xavier_uniform_(emb.weight.data)
            self.embeds.append(emb)
        self.enc_norm = nn.LayerNorm(embedding_dim)
         # GNN layer
        self.gnn = GATConv(embedding_dim, gnn_output_dim)
        self.act = nn.LeakyReLU()
        # MLP layers
        self.mlp = nn.Sequential(
            nn.Linear(gnn_output_dim, mlp_hidden_dim),
            nn.LeakyReLU(),
            nn.Linear(mlp_hidden_dim, mlp_output_dim),
            nn.LeakyReLU(),
        )

        self.atom_dict, _, _ = gpickle.load(atom_dict_file)
    
    def _compoundfeatures(self, dataset: Tensor,  aux_data_list=None) -> Tensor:
        """
        Corresponding to input is lists of the compound smiles
        :param dataset: lists of SMILES
        :param atom_dict:
        :param batch_sample_idx:
        :return: torch_geometric format batch data 
        """
        ...
        for item_idx in range(batch_size):
            ...
            try: # input value: valid SMILES
                ...
                for atom in molecule.GetAtoms():
                    if atom.GetSymbol() not in self.atom_dict:
                ...
                n_compound = len(tokenized_seq_compound)
            ...
            edge_list_reverse_compound = [(j, i) for (i, j) in edge_list_compound]
            edge_list_compound.extend(edge_list_reverse_compound)
            edges_compound = edge_list_compound
            nodes_compound = torch.tensor(tokenized_seq_compound, dtype=float)

            edges_compound = np.array(edges_compound).T
            edges_compound = torch.tensor(edges_compound, dtype=torch.long)
            sample_data_compound = Data(x=nodes_compound,edge_index=edges_compound)
            batch_data_compound.append(sample_data_compound)
    ...
    def forward(self,  x: Tensor, seq_len: int, device: torch.device) -> Tensor:
        ...
        for data in data_list:
            x = data.x.long()  # Convert node features to long type
            x_embed = self.embeds[0](x[:, 0])
            for i in range(1, min(len(self.embeds), x.shape[1])):
                x_embed += self.embeds[i](x[:, i])
            x_embed = x_embed.to(device)
            x_embed = self.enc_norm(x_embed)
            data.edge_index = data.edge_index.to(device)
            
            # Apply GAT layer
            x_gnn = self.gnn(x_embed, data.edge_index)
            x_gnn = self.enc_norm(x_gnn)
            x_gnn = self.act(x_gnn)
            
            x_gnn_1 = self.gnn(x_gnn, data.edge_index)
            x_gnn_1 = self.enc_norm(x_gnn_1)
            x_gnn_1 = self.act(x_gnn_1)
...
class ExprDecoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        ...
        second_linear_output = self.fc[1](x).squeeze(-1)  # (batch, seq_len)
        third_linear_output = self.fc[2](x).squeeze(-1)  # (batch, seq_len)
        
        pred_value = self.fc(x).squeeze(-1)  # (batch, seq_len)

        if not self.explicit_zero_prob:
            return first_linear_output, second_linear_output, third_linear_output, dict(pred=pred_value)
        zero_logits = self.zero_logit(x).squeeze(-1)  # (batch, seq_len)
        zero_probs = torch.sigmoid(zero_logits)
        return first_linear_output, second_linear_output, third_linear_output, dict(pred=pred_value, zero_probs=zero_probs)
...
class SiameseNetwork(nn.Module):
    ...
    def forward_one(self, input_gene_ids, input_values, smiles_comps, src_key_padding_mask):
        ...
        features_gat, features_transfomer, features_mlp, second_features_mlp, third_features_mlp, output_dict = self.transformer(input_gene_ids, input_values, smiles_comps, src_key_padding_mask)
        prediction = output_dict
        return features_gat, features_transfomer, features_mlp, second_features_mlp, third_features_mlp, prediction
    ...
    def forward(self, input1, input2):
        ...
        features_gat1, features_transfomer1, features_mlp1, second_features_mlp1, third_features_mlp1, prediction1 = self.forward_one(input_gene_ids1, input_values1, smiles_comps1, src_key_padding_mask1)
        features_gat2, features_transfomer2, features_mlp2, second_features_mlp2, third_features_mlp2, prediction2 = self.forward_one(input_gene_ids2, input_values2, smiles_comps2, src_key_padding_mask2)
        return features_gat1, features_transfomer1, features_mlp1, second_features_mlp1, third_features_mlp1, features_gat2, features_transfomer2, features_mlp2, second_features_mlp2, third_features_mlp2, prediction1, prediction2
