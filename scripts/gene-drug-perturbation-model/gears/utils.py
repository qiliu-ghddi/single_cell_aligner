...
def make_GO(data_path, pert_list, data_name, num_workers=25, save=True):
    ...
    fname = '/home/zqliu02/code/share/GO/go.csv'
...
def create_cell_graph_for_prediction(X, pert_idx, pert_gene,pert_flags,drug_pert=None,):

    if pert_idx is None:
        pert_idx = [-1]
   
    
    return Data(x=torch.Tensor(X).T, pert_idx = pert_idx, pert=pert_gene,pert_flags=pert_flags,drug_pert = drug_pert,)
    

def create_cell_graph_dataset_for_prediction(gene_pert_idx, pert_gene, ctrl_adata, gene_names,
    ...
    try:
        if ctrl_adata.X.toarray().shape[0]<=1000:
            Xs = ctrl_adata.X.toarray()
        else:
            
            selected_indices = np.random.choice(ctrl_adata.X.toarray().shape[0], 1000, replace=False)
            Xs = ctrl_adata.X.toarray()[selected_indices]

    except:
        if ctrl_adata.X.shape[0]<=1000:
            Xs = ctrl_adata.X
        else:   
            selected_indices = np.random.choice(ctrl_adata.X.shape[0], 1000, replace=False)
            Xs = ctrl_adata.X[selected_indices]

    
    pert_flags = np.zeros(len(ctrl_adata.var["gene_name"].tolist()))
    for i in range(0,len(gene_pert_idx)):
        if len(pert_idx) == 1:
            if gene_pert_idx[i][1] == pert_idx[0]:
                pert_flags[gene_pert_idx[i][0]]=1
        ...
        if len(pert_idx) == 2:
            ...
            if gene_pert_idx[i][1] == pert_idx[1]:
                pert_flags[gene_pert_idx[i][0]]=1
           

    #test end
    # Create cell graphs
    cell_graphs = [create_cell_graph_for_prediction(X, pert_idx, pert_gene,pert_flags,).to(device) for X in Xs]
    return cell_graphs

def create_drug_cell_graph_dataset_for_prediction( pert, ctrl_adata, device, num_samples = 300):
    ...
    pert_flags =None
    pert_idx =None
    # if pert.rpartition('+')[0]=="ctrl":
    #     drug_pert = pert.rpartition('+')[2]
    # elif pert.rpartition('+')[2]=="ctrl":
    #     drug_pert = pert.rpartition('+')[0]
           

    #test end
    # Create cell graphs
    cell_graphs = [create_cell_graph_for_prediction(X, pert_idx, pert,pert_flags,pert[0]).to(device) for X in Xs]
    return cell_graphs
##
##GI related utils
##
