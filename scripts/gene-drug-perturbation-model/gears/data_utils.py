...
def get_DE_genes(adata, skip_calc_de,model_type):
    if model_type == "drug":
        ...
        adata.obs.loc[:, 'condition_name'] =  adata.obs.apply(lambda x: '_'.join([x.cell_type, x.condition.replace("\\", "_").replace("/", "_").replace("*", "_"), x.dose_val]), axis = 1) 
...
def get_dropout_non_zero_genes(adata):
    ...
    top_non_dropout_de_50 = {}
    top_non_dropout_de_100={}
    top_non_dropout_de_200 = {}
    top_non_dropout_de_all = {}
    ...
    for pert in adata.uns['rank_genes_groups_cov_all'].keys():
        ...
        #.X.toarray
        try:
            X = adata[adata.obs.condition == p].X.toarray()
        except:
            X = adata[adata.obs.condition == p].X
        ...
        non_dropout_50 = [i for i in gene_idx_top if i in non_dropouts][:50]
        non_dropout_100 = [i for i in gene_idx_top if i in non_dropouts][:100]
        non_dropout_200 = [i for i in gene_idx_top if i in non_dropouts][:200]
        non_dropout_all = [i for i in gene_idx_top if i in non_dropouts]

        non_dropout_20_gene_id = [gene_idx2id[i] for i in non_dropout_20]
        non_dropout_50_gene_id = [gene_idx2id[i] for i in non_dropout_50]
        non_dropout_100_gene_id = [gene_idx2id[i] for i in non_dropout_100]
        non_dropout_200_gene_id = [gene_idx2id[i] for i in non_dropout_200]
        non_dropout_all_gene_id = [gene_idx2id[i] for i in non_dropout_all]
        ...
        non_zero_50_gene_id = [gene_idx2id[i] for i in non_zero_50]
        non_zero_100 = [i for i in gene_idx_top if i in non_zero][:100]
        non_zero_100_gene_id = [gene_idx2id[i] for i in non_zero_100]
        non_zero_200 = [i for i in gene_idx_top if i in non_zero][:200]
        non_zero_200_gene_id = [gene_idx2id[i] for i in non_zero_200]
        non_zero_all = [i for i in gene_idx_top if i in non_zero]
        non_zero_all_gene_id = [gene_idx2id[i] for i in non_zero_all]
        ...
        top_non_dropout_de_50[pert] = np.array(non_dropout_50_gene_id)
        top_non_zero_de_50[pert] = np.array(non_zero_50_gene_id)
        top_non_dropout_de_100[pert] = np.array(non_dropout_100_gene_id)
        top_non_zero_de_100[pert] = np.array(non_zero_100_gene_id)
        top_non_dropout_de_200[pert] = np.array(non_dropout_200_gene_id)
        top_non_zero_de_200[pert] = np.array(non_zero_200_gene_id)
        top_non_dropout_de_all[pert] = np.array(non_dropout_all_gene_id)
        top_non_zero_de_all[pert] = np.array(non_zero_all_gene_id)
    ...
    adata.uns['top_non_dropout_de_50'] = top_non_dropout_de_50
    adata.uns['top_non_zero_de_50'] = top_non_zero_de_50
    adata.uns['top_non_dropout_de_100'] = top_non_dropout_de_100
    adata.uns['top_non_zero_de_100'] = top_non_zero_de_100
    adata.uns['top_non_dropout_de_200'] = top_non_dropout_de_200
    adata.uns['top_non_zero_de_200'] = top_non_zero_de_200

    adata.uns['top_non_dropout_de_all'] = top_non_dropout_de_all
    adata.uns['top_non_zero_de_all'] = top_non_zero_de_all
